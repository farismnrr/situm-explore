...
import { normalizeSitumRealtimeFeatures, normalizeSitumRealtimeFeaturesWithStats } from '../server/utils/situm-realtime'
...
test('Realtime normalization reports bounded accepted and dropped counts', () => {
  const result = normalizeSitumRealtimeFeaturesWithStats([
    { properties: { time: '2026-08-18T06:47:48.979Z', buildingId: 19866, floorId: 69905, accuracy: 2 }, geometry: { coordinates: [106.8, -6.1] } },
    { properties: { time: 'not-a-date', buildingId: 19866, floorId: 69905, accuracy: 2 }, geometry: { coordinates: [106.8, -6.1] } },
  ])
  assert.equal(result.inputCount, 2)
  assert.equal(result.acceptedCount, 1)
  assert.equal(result.droppedCount, 1)
  assert.deepEqual(result.positions, normalizeSitumRealtimeFeatures([
    { properties: { time: '2026-08-18T06:47:48.979Z', buildingId: 19866, floorId: 69905, accuracy: 2 }, geometry: { coordinates: [106.8, -6.1] } },
    { properties: { time: 'not-a-date', buildingId: 19866, floorId: 69905, accuracy: 2 }, geometry: { coordinates: [106.8, -6.1] } },
  ]))
})
