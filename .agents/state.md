...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`, with Plan 032 active on `plan/032-web-native-handoff-distribution` and `plans/033-full-e2e-acceptance.md` terminal. Plan 032 Phases 0–6 are complete: the 768×600 Map Viewer capability threshold, environment-only mobile distribution configuration, native Map/Realtime deep-link parsing, pending-login restoration, workspace reauthorization, shared web Native App Gate, geometry-gated Map policy, all-web Realtime handoff, deterministic app-link/QR/install fallback policy, and reproducible distribution documentation are implemented. Phase 7 remains in progress; real device/cross-client behavior remains Plan 033.
