...
There is currently **no active implementation plan**. New work must start from updated `main` on a new dedicated plan branch. Historical plans, sessions, evidence, and retired branches are context only unless explicitly re-opened.
