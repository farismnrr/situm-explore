...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`. Plan 032 implementation exists through `faad63e` on `plan/032-web-native-handoff-distribution`, but final reviewer remediation is required before PR readiness. Two non-device blockers are authoritative in `.agents/reviews/plan-032-final.md`: deep-link `requestedBuildingId` can survive beyond its intended use and leak across later workspace/navigation changes, and `NativeAppGate` incorrectly derives install-platform choice from viewport width so Android tablets/desktop can receive iOS-only fallback actions. All physical-device and cross-client acceptance remains explicitly unpassed for Plan 033; Plan 033 has not started.
