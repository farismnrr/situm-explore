...
Plan 041 — Navigation-First Native Map UI is active on `plan/041-navigation-first-map-ui`, based on current `origin/main`. PHASE-01 is complete: a clean mobile install verified the exact `@situm/react-native@3.19.2` source contract for navigation progress/indications and MapView controls; narrow local types plus deterministic distance/ETA/indication helpers are in place and focused tests pass. Mobile lint initially exposed that `mobile/eslint.config.mjs` imported `@typescript-eslint/parser` without declaring it; the missing direct dev dependency was added and mobile lint/typecheck now pass. PHASE-02 browse/search/floor/location presentation is next. The connected Android POS at `100.113.52.76:37371` is the physical acceptance target. No PR, merge, OTA publication, or production deployment is authorized by this plan.
