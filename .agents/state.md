...
Plan 029 Phase 3 implementation requires remediation before acceptance: reviewer inspection found that `mobileSessionConfig()` omits the nuxt-auth-utils session name before direct `h3.sealSession()`, so the returned mobile token can seal the wrong `h3` session bucket; version validation also fails open for legacy/unversioned sessions. Focused regression coverage for the new mobile auth/revocation boundary is missing. Dedicated encrypted Positioning credential persistence/owner-checked issuance exists but remains part of the remediation acceptance pass.
...
Plan 029 Phase 5 implementation also requires remediation: reviewer inspection found phone navigation is not laid out as the approved horizontal bottom nav, there is no distinct compact tablet/POS rail, the canonical brand/Lucide icon language is not followed, foreground `AppState` is incorrectly labeled `Online`, and workspace selection is not reliably reactive/restored across restart. Map/Realtime remain truthful placeholders and no positioning runtime has started.
