...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`. Plan 032 implementation and final reviewer remediation are complete on `plan/032-web-native-handoff-distribution`, pending final review/integration. The two non-device blockers from `.agents/reviews/plan-032-final.md` are resolved: deep-link building hints are one-shot, and NativeAppGate install options use OS detection independent of viewport. All physical-device and cross-client acceptance remains explicitly unpassed for Plan 033; Plan 033 has not started.
