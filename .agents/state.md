...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`, with Plan 032 active on `plan/032-web-native-handoff-distribution` and `plans/033-full-e2e-acceptance.md` terminal. Plan 032 Phases 0–4 are complete: the 768×600 Map Viewer capability threshold, environment-only mobile distribution configuration, native Map/Realtime deep-link parsing, pending-login restoration, workspace reauthorization, shared web Native App Gate, geometry-gated Map policy, and all-web Realtime handoff policy are implemented. Phases 5–7 remain in progress; real device/cross-client behavior remains Plan 033.
