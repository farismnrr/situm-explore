...
The Docker base-image warning is addressed in the active Plan 038 worktree: both build stages now use Node `22.22.0-bookworm-slim`, and a clean production image build completed without the Nuxt engine mismatch warning. The current container is deployed from `ccb8e0a`, before the follow-up Viewer lifecycle fix; it will be rebuilt after that fix. Authenticated browser acceptance then isolated the Viewer issue to immediate watcher timing before the DOM ref existed; `SitumViewer` now retries in `onMounted`, with a regression assertion and targeted test passing. The authenticated staging browser tab is available through `https://situm.farismunir.my.id` (the local port-3005 tunnel) and will be rechecked after final recreate. `adb devices` currently reports no Android device, so physical positioning remains an explicit pending gate.
