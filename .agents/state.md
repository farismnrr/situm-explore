...
Plan 031 runtime remediation at `79b6cfb` resolves the prior three reviewer blockers: caller cancellation now reaches `fetch`, unsupported local freshness thresholds were removed, and malformed Realtime payloads fail closed. Focused regressions and full validation pass. One final visual-truthfulness blocker remains before PR readiness: every Realtime row still renders an unsupported success-green status dot even though the contract has no per-record online/fresh/healthy state. Remove or neutralize that semantic indicator per `.agents/reviews/plan-031-final.md`. Baseline v1 semantics remain device-position oriented: the existing owner-scoped Nitro Realtime route returns position/device identity, source time, building/floor, accuracy, coordinates, and optional device ID. Do not invent person identities, freshness/presence state, trajectories, remote markers/focus, or treat Share Live Location as Realtime Positions.
