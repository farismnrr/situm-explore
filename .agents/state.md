...
Plan 030 implementation exists on `plan/030-native-map-positioning-navigation`, but final reviewer remediation is required before phases 0–6 can be accepted. Blocking findings are recorded in `.agents/reviews/plan-030-final.md`: unauthenticated `SitumProvider` render deadlock, workspace-switch stale credential/building exposure, non-truthful current-location freshness/directions gating, incomplete navigation teardown, fabricated Map-selected POI details, and a dead-end map-data retry path. Root tests/diff checks pass and `expo-doctor` remains 20/21 with only the known Situm metadata warning. Mandatory physical acceptance also remains externally blocked: `adb devices` reports no connected device, `mobile/.env` is absent, and no owner-authorized calibrated building/profile runtime evidence is available. Do not claim real blue-dot, floor-transition, POI, or navigation acceptance; do not create a PR or start Plan 031.
