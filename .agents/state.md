...
Plan 031 implementation is remediated and PR-ready on `plan/031-native-realtime-operations` after the final review findings were fixed: caller cancellation now reaches `fetch`, local freshness thresholds were removed, and malformed Realtime payloads fail closed. Focused regressions and full validation passed. Baseline v1 semantics remain device-position oriented: the existing owner-scoped Nitro Realtime route returns position/device identity, source time, building/floor, accuracy, coordinates, and optional device ID. Do not invent person identities, freshness/presence state, trajectories, remote markers/focus, or treat Share Live Location as Realtime Positions.
