...
Plan 041 — App-Owned Indoor Map + Navigation UI is active on `plan/041-navigation-first-map-ui`, based on current `origin/main`. The first MapView-overlay candidate was physically rejected because the center map still looked like Situm; the plan pivoted to a genuine app-owned renderer. `NativeMapScreen` now consumes authenticated cartography + path graph endpoints, renders the real floorplan through React Native Image with app-owned SVG POIs/bluedot/accuracy/heading/route overlays, and calculates route geometry from the venue graph. Situm native is retained only as the shared headless foreground positioning producer. Physical Metro E2E on the connected POS exposed and fixed three runtime defects that source-only validation missed: the headless path initializes Situm before configuration, POI selection dismisses the Android keyboard so destination actions remain tappable, and fullscreen no longer remounts Explore/loses active route state. Follow-up hardening moved Situm initialization into the explicit fail-closed positioning start path so an SDK-init failure cannot crash the authenticated shell, removed redundant map-screen listener registration, and corrected stale Settings copy. A fresh POS rerun received real fixes and Realtime remained healthy. The 92/92 Node regression/unit result is retained only as historical pre-policy evidence. By explicit user policy on 2026-09-02, all repo-authored persistent unit/regression test files and test package scripts are removed; current durable validation uses lint/typecheck/diff/build plus real runtime/browser/device evidence, while any E2E/white-box/black-box test artifact must be temporary and deleted before commit/closeout. The arm64 debug shell is installed and Metro remains the approved JS/TS iteration loop. The final post-E2E arm64 release package also passes through the repository wrapper: `situm-explore-v1.0.2-android-arm64.apk`, SHA-256 `60ed6a8b5a589364dc61497bf2c0c7c05926fa417586885aba9467d853d9b165`, checksum verified, arm64-only native libraries. The branded Android launch is accepted on the final release artifact: cold launch resumes without fatal/React errors and logo-only frame analysis proves the app logo pulses from 88×88 px to 95×94 px before settling near 90×89 px. Pixel-level map screenshot polish and movement-dependent arrival/off-route reproduction remain separate physical follow-ups and are not falsely claimed. Auto-commit is authorized locally; no push, PR, merge, OTA publication, or production deployment is authorized by this plan.
