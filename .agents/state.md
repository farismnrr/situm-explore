...
Plan 030 first remediation at `74b9a99` resolved the original six reviewer blockers. The second remediation resolves the remaining navigation-truthfulness/lifecycle blockers: out-of-route UI now reports only the observed condition, and positioning stop/error/background/building/runtime teardown cancels navigation only when navigation is actually owned or native-running. Root tests pass 17/17; root/mobile validation and Android assembleDebug pass. Expo Doctor reports 19/21 because the known Situm New Architecture metadata warning remains and the plan-frozen Expo patch versions are behind Doctor's latest patch expectations; that patch drift is not itself a blocker under current frozen authority. Mandatory physical acceptance remains externally blocked: `adb devices` is empty, `mobile/.env` is absent, and no owner-authorized calibrated building/profile runtime evidence is available. Do not claim real blue-dot, floor-transition, POI, or navigation acceptance; do not create a PR or start Plan 031.
