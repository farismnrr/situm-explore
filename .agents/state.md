...
Plan 030 implementation through Phase 6 is reviewer-approved on `plan/030-native-map-positioning-navigation` after remediations `74b9a99` and `8afb39b`. The original six blockers and the final navigation-truthfulness/lifecycle blockers are resolved: credentialed provider ownership, workspace-keyed teardown, fresh/stale/error current-location gating, real POI resolution, functional retry, and conditional navigation cancellation across stop/error/background/building/runtime teardown. Independent reviewer checks pass: root tests 17/17, root/mobile lint and typecheck, `git diff --check`, and clean/synchronized branch; reported Expo prebuild and Android assembleDebug evidence remains consistent. Expo Doctor reports 19/21 because the known Situm New Architecture metadata warning remains and the plan-frozen Expo patch versions are behind Doctor's latest patch expectations; that patch drift is not itself a blocker under current frozen authority. Plan 030 is still NOT PR-ready because mandatory physical-device Phase 7 acceptance remains externally blocked: `adb devices` is empty, `mobile/.env` is absent, and no owner-authorized calibrated building/profile runtime evidence is available. Do not claim real blue-dot, floor-transition, POI, or navigation acceptance; do not create a PR or start Plan 031.
...
Plan 030 owns the production native MapView, foreground positioning/blue dot, building/floor/POI interaction, evidence-backed directions/navigation, lifecycle cleanup, and real-device acceptance. Plan 031 still owns native Realtime; do not implement Realtime or Share Live Location in Plan 030.

Physical-device evidence is mandatory before claiming real positioning, blue-dot, floor-transition, or navigation acceptance. Lack of a suitable calibrated building/profile, owner-authorized Positioning credential, or supported physical device is an explicit acceptance gate, not permission to fabricate runtime evidence.
...
- The native package is standalone under `mobile/` with no npm workspaces; Plan 029 implemented and integrated that foundation.
...
- Plan 028 Phase 6 completed the final matrix and capability-to-reference handoff. Plan 029 then implemented and integrated the foundation; Plan 030 now owns native Map/positioning/navigation while iOS/macOS, public HTTPS association, and release signing/store delivery remain explicit external gates.
