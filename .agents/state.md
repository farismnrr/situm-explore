...
Plan 031 implementation exists through `42bf3f1` on `plan/031-native-realtime-operations`, but final reviewer remediation is required before implementation approval/PR readiness. Three non-device blockers are authoritative in `.agents/reviews/plan-031-final.md`: caller cancellation is currently swallowed by `MobileApiClient` before reaching `fetch`, local 60s/5m freshness thresholds fabricate unsupported semantics, and malformed Realtime payloads can be normalized into misleading empty/partial results. Baseline v1 semantics remain device-position oriented: the existing owner-scoped Nitro Realtime route returns position/device identity, source time, building/floor, accuracy, coordinates, and optional device ID. Do not invent person identities, online/idle/offline state, trajectories, remote markers/focus, or treat Share Live Location as Realtime Positions.
