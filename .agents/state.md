...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`, with Plan 032 implementation complete on `plan/032-web-native-handoff-distribution` pending review/integration, and `plans/033-full-e2e-acceptance.md` terminal. Plan 032 Phases 0–7 are complete: geometry-gated Map handoff, all-web Realtime handoff, authenticated deep links, deterministic app-link/QR/install fallback, distribution documentation, and full available non-device validation are recorded in `.agents/evidence/plan-032.md`. All physical-device and cross-client acceptance remains explicitly unpassed for Plan 033; Plan 033 has not started.
