...
Current planning authority is `plans/028-033-native-mobile-roadmap.md`. Plan 032 remains in final reviewer remediation on `plan/032-web-native-handoff-distribution`. The two prior non-device blockers were resolved in `ab35054`; the foreground lifecycle remediation now emits monotonic Map requests and applies sequential same-workspace building links exactly once without a parent remount loop. Final reviewer confirmation and integration remain outstanding. All physical-device and cross-client acceptance remains explicitly unpassed for Plan 033; Plan 033 has not started.
