...
Plan 030 reviewer remediation is complete on `plan/030-native-map-positioning-navigation`: SitumProvider now receives the owner-authorized Positioning key before SDK children render; workspace changes tear down the keyed native runtime; current-location freshness and directions are bounded to the active workspace/building; navigation cancellation, lifecycle cleanup, real POI resolution, and retry/refetch are covered. Root/mobile validation and x86_64-capable Android assembleDebug pass. Expo Doctor reports 19/21 because the known Situm New Architecture metadata warning remains and the plan-frozen Expo patch versions are behind Doctor's latest patch expectations. Mandatory physical acceptance remains externally blocked: `adb devices` is empty, `mobile/.env` is absent, and no owner-authorized calibrated building/profile runtime evidence is available. Do not claim real blue-dot, floor-transition, POI, or navigation acceptance; do not create a PR or start Plan 031.
