...
export function RealtimeScreen({ workspaces, lifecycle, positioning, layout = 'phone' }: { workspaces: WorkspaceContext, lifecycle: string, positioning: ForegroundPositioningSession, layout?: LayoutMode }) {
  ...
  const [workspaceBuildings, setWorkspaceBuildings] = useState<{ id: number, name: string }[]>([])
  ...
  const ownBuildingId = positioningSnapshot.buildingId ?? workspaceBuildings[0]?.id ?? null

  useEffect(() => {
    if (!workspaceId) { setWorkspaceBuildings([]); return }
    const controller = new AbortController()
    void workspaces.auth.api.get<{ buildings: { id: number, name: string }[] }>(`/api/workspaces/${workspaceId}/situm/cartography`, { signal: controller.signal, timeoutMs: 10_000 }).then(result => { if (!controller.signal.aborted) setWorkspaceBuildings(result.buildings) }).catch(() => { if (!controller.signal.aborted) setWorkspaceBuildings([]) })
    return () => controller.abort()
  }, [workspaceId, workspaces])

  const startOwnLocation = useCallback(() => { if (!workspaceId || ownBuildingId === null) return; void positioning.start(workspaceId, ownBuildingId, () => workspaces.getPositioningCredential()) }, [ownBuildingId, positioning, workspaceId, workspaces])
  const stopOwnLocation = useCallback(() => positioning.stop('explicit'), [positioning])
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
    ...
    <OwnDeviceLocationCard snapshot={positioningSnapshot} buildingName={workspaceBuildings.find(building => building.id === ownBuildingId)?.name ?? null} canStart={ownBuildingId !== null} onStart={startOwnLocation} onStop={stopOwnLocation} />
    ...
    {state === 'empty' || (state === 'error' && !positions.length) ? <StateCard title={state === 'error' ? 'Realtime unavailable' : 'No positions reported'} body={state === 'error' ? (error || 'Check your connection and try again.') : 'This workspace has no current device-position records.'} action={state === 'error' ? () => setRefreshNonce(value => value + 1) : undefined} /> : <View style={[styles.columns, layout !== 'phone' && styles.columnsWide]}>
...
function OwnDeviceLocationCard({ snapshot, buildingName, canStart, onStart, onStop }: { snapshot: ReturnType<ForegroundPositioningSession['getSnapshot']>, buildingName: string | null, canStart: boolean, onStart: () => void, onStop: () => void }) {
  const active = snapshot.state === 'active'
  const starting = snapshot.state === 'starting'
  const location = snapshot.location
  const floor = location?.position?.floorIdentifier ? `Floor ${location.position.floorIdentifier}` : null
  return <View style={styles.ownDevice}><View style={styles.ownDeviceCopy}><Text style={styles.eyebrow}>THIS DEVICE</Text><Text style={styles.ownDeviceTitle}>{active ? 'Live location active' : starting ? 'Starting live location…' : snapshot.state === 'error' ? 'Live location needs attention' : 'Live location is off'}</Text><Text style={styles.muted}>{active ? [buildingName, floor].filter(Boolean).join(' · ') || snapshot.message : snapshot.message}</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel={active || starting ? 'Stop live location' : 'Enable live location'} accessibilityState={{ disabled: !canStart && !active && !starting }} disabled={!canStart && !active && !starting} onPress={active || starting ? onStop : onStart} style={[styles.locationAction, !canStart && !active && !starting && styles.disabled]}><Text style={styles.locationActionText}>{active || starting ? 'Stop location' : 'Enable live location'}</Text></TouchableOpacity></View>
}
