...
export class ForegroundPositioningSession {
  ...
  private onStatus(status: LocationStatus) { if (status.statusName === 'STOPPED') return this.stopFromNative('Location is stopped.'); if (status.statusName === 'USER_NOT_IN_BUILDING') return this.update({ ...this.snapshot, state: 'error', location: null, receivedAt: null, message: 'Situm could not determine a position in the selected building.' }); if (this.snapshot.state !== 'stopped') this.update({ ...this.snapshot, state: 'starting', message: 'Situm is determining your indoor position…' }) }
  private onError(_error: SitumError) { if (this.snapshot.state !== 'stopped') this.update({ ...this.snapshot, state: 'error', location: null, receivedAt: null, message: 'Situm could not determine a position.' }) }
