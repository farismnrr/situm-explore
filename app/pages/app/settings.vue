...
<template>
  <div class="space-y-6">
    ...
    <UAlert v-if="resetMessage" color="info" variant="soft" :description="resetMessage" close @update:open="resetMessage = ''" />
    <UAlert color="info" variant="soft" title="Local-only settings" description="These preferences are stored in this page for the current session. No Situm settings, styles, images or configuration are read or changed." />

    <div class="grid gap-6 lg:grid-cols-[13.75rem_minmax(0,1fr)]">
      ...
      <UCard :ui="{ body: 'p-0 sm:p-0' }" class="overflow-hidden">
        <section v-if="activeTab === 'general'" class="p-5 sm:p-6">
          ...
          <div class="mt-5 divide-y divide-default">
            <div class="flex items-center justify-between gap-5 py-4 first:pt-0"><div><strong class="block text-sm text-highlighted">Light UI mode</strong><span class="mt-1 block text-xs leading-relaxed text-muted">Keep Situm Explore light-only for this product phase.</span></div><USwitch :model-value="true" disabled aria-label="Light UI mode locked on" /></div>
            <div class="flex items-center justify-between gap-5 py-4"><div><strong class="block text-sm text-highlighted">Show user settings</strong><span class="mt-1 block text-xs leading-relaxed text-muted">Expose viewer accessibility controls.</span></div><USwitch v-model="showUserSettings" aria-label="Show user settings" /></div>
            <div class="flex items-center justify-between gap-5 py-4"><div><strong class="block text-sm text-highlighted">Follow user by default</strong><span class="mt-1 block text-xs leading-relaxed text-muted">Center the map on the selected realtime position.</span></div><USwitch v-model="followUser" aria-label="Follow user by default" /></div>
            <div class="flex items-center justify-between gap-5 py-4 last:pb-0"><div><strong class="block text-sm text-highlighted">Language</strong><span class="mt-1 block text-xs leading-relaxed text-muted">Map Viewer interface language.</span></div><USelect v-model="language" :items="['English', 'Bahasa Indonesia']" aria-label="Map Viewer interface language" class="w-44" /></div>
          </div>
        </section>

        <section v-else-if="activeTab === 'navigation'" class="p-5 sm:p-6">
        ...
        <section v-else-if="activeTab === 'map'" class="p-5 sm:p-6">
        ...
        <section v-else-if="activeTab === 'styles'" class="p-5 sm:p-6">
