...
<template>
  <div class="operations-page space-y-6">
    ...
    <UCard v-else-if="String(status) === 'success' && !error" :ui="{ body: 'p-0 sm:p-0' }" class="overflow-hidden">
      ...
      <p v-if="String(status) === 'success' && !error && alarms.length === 0" class="px-5 py-10 text-center text-sm text-muted">No Situm alarms match the selected building and filters.</p>
    </UCard>
