...
type SyncInput = { report: AnalyticsReport, fromDate: string, toDate: string, buildingId?: number, buildingIds?: number[], timeZone?: string, grouping?: string }
...
export async function syncSitumReport(input: SyncInput) {
  ...
  const syncKey = buildAnalyticsSyncKey(input.report, input.fromDate, input.toDate, scope)
  ...
  const response = await fetch(`https://api.situm.com/api/v1/reports/${paths[input.report]}.json?${params}`, { headers: { 'X-API-KEY': config.situmApiKey } })
  if (!response.ok) errorFor(response.status)
  const rows = asRows(await response.json() as ReportResponse)
  const client = getClickHouseClient()
  const table = input.report === 'visitors' ? 'analytics_visitors' : input.report === 'positioning_time' ? 'analytics_positioning_time' : 'analytics_geofencing_stay'
  ...
  try {
    ...
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(database) || !/^analytics_(visitors|positioning_time|geofencing_stay)$/.test(table)) throw new Error('Invalid analytics table configuration')
    await client.command({
      query: `ALTER TABLE \`${database}\`.${table} DELETE WHERE source_window_id = {source_window_id:String}`,
      query_params: { source_window_id: syncKey },
      clickhouse_settings: { mutations_sync: '2' }
    })
    if (normalized.length) await client.insert({ table, values: normalized, format: 'JSONEachRow' })
    const now = new Date().toISOString()
    await client.insert({ table: 'analytics_sync_runs', values: [{ sync_key: syncKey, report: input.report, from_date: `${input.fromDate}T00:00:00.000Z`, to_date: `${input.toDate}T23:59:59.999Z`, scope, status: 'complete', started_at: now, completed_at: now, updated_at: now }], format: 'JSONEachRow' })
