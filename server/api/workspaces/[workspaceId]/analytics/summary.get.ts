...
export default defineEventHandler(async (event) => {
  ...
  try { await ensureClickHouseSchema(); return await queryWorkspaceAnalytics(workspaceId, { fromDate, toDate }) } catch { throw createError({ statusCode: 503, statusMessage: 'Analytics data could not be read from ClickHouse.' }) }
})
