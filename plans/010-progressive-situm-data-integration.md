...
| Trajectory | UNRESOLVED | SDK `reports.getTrajectory({ fromDate, toDate, buildingId, userId? })`; Viewer trajectory loading declarations are verified, but product mapping, permissions, response handling and stale/empty behavior remain unverified | Future authenticated Nitro report read; Viewer rendering only after exact UI mapping | timestamp/session/floor/user/device/lat/lng are candidate fields, not yet an implementation contract | Plan 013/014; absent as working UI |
| Reports beyond trajectory and CSV | UNRESOLVED | Official REST lists `/api/v1/reports/geofencing_stay_time.{format}`, `/geofencing_sessions_matches.{format}`, `/visitors.{format}`, `/positioning_time.{format}`, `/heatmap.{format}`, `/user_positions.{format}`, `/map_viewer.{format}`, and `/raw_data.{format}`; SDK 0.25.0 does not declare these methods | Future authenticated Nitro only | exact filters, response schemas, permissions, format handling and UI mapping remain unverified | Plan 014; absent as working UI |
...
| Groups | UNRESOLVED | Official REST lists `GET /api/v1/groups`; installed SDK exposes no `GroupsApi`, group method or response type | Future authenticated Nitro only if reverified | exact read fields, permissions and pagination remain unverified; no fixture rows are presented as current data | Plan 015 |
| Alarms | UNRESOLVED | Official REST lists `GET /api/v1/alarms` and `GET /api/v1/alarms/{id}` plus status operations; installed SDK exposes no alarm API/types | Future authenticated Nitro only if reverified | exact read schema, filters and permissions remain unverified; no fixture rows are presented as current data | Plan 015 |
| Location picker/camera/language/search/accessibility/favorites | WEB / SITUM | Viewer `openLocationPicker`, `setCamera`, `setLanguage`, `setSearchFilter`, `setFavoritePois`, `showUserSettings`, `updateFontSize`; `LOCATION_PICKER_DRAGGED` and map events | Browser Viewer; exact command wiring remains later work | method declarations and event fields are verified; `setConfigItem(key, value)` options remain unconstrained and are not treated as verified settings | Plan 016 |
