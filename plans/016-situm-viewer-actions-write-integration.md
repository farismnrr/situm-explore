...
Plan 010 must provide an explicit retained list before this plan starts.
...
- REST-backed configuration reads/writes, if any are explicitly retained, go through authenticated Nitro routes with private server credentials.
- A Read-Write server credential must never be exposed to browser code.
...
Expose only the smallest typed command surface needed by retained UI rather than creating independent SDK clients across pages. Example shape only:
...
Do not expose a giant generic `invokeViewer(method, payload)` escape hatch.

## Phase 1 — Freeze exact scope

- [ ] Confirm Plan 015 is integrated.
- [ ] Read Plan 010 final capability matrix.
...
- [ ] Classify each as direct Viewer action vs authenticated Nitro REST operation.

## Phase 2+ — Implement one capability group at a time

For each selected group:

- [ ] wire the real capability;
- [ ] remove the replaced dummy/local success behavior;
